...
<style>
    /* Ensure background covers entire viewport without awkward short scroll */
    body {
        margin: 0;
    ...
    .password-wrapper {
        max-width: 400px;
        margin: 130px auto 80px; /* leaves space below navbar; pushes box down a bit */
    ...
    /* Prevent tiny accidental page scroll when content shorter than viewport */
    html, body { overscroll-behavior: contain; }
    @media (max-width:576px){
         .password-wrapper { margin:110px 12px 60px; padding:22px 20px 26px; }
         .password-wrapper h1 { font-size:24px; }
    }
</style>
...
<div class="password-wrapper">
    ...
    if(isset($_POST['submit'])) {
            $rest = mysqli_query($db,"SELECT * FROM `admin` WHERE username='$_POST[username]' AND email='$_POST[email]'");
     $_SESSION['username'] = $_POST['username'];
    $_SESSION['password'] = $_POST['password'];
    $_SESSION['email'] = $_POST['email'];
    if($rest) { //  UPDATE  
        if(mysqli_affected_rows($db) > 0) { //   affected   
...
<!-- Body & HTML closing tags already printed in navbar include on other pages; keep page lean -->