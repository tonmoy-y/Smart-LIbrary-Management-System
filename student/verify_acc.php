...
<body class="bgs">
    <div class="box1" style="padding: 30px;">
        <br>
        <h3 style="text-align: center; text-weight=700;">Account Verification</h3>
...
if (isset($_POST['submit_v'])) {
    ...
    $stmt = mysqli_prepare($db, "SELECT * FROM verify WHERE username=? AND otp=? LIMIT 1");
    ...
    if ($res && mysqli_num_rows($res) === 1) {
        ...
        $delStmt = mysqli_prepare($db, "DELETE FROM verify WHERE username=?");
        mysqli_stmt_bind_param($delStmt, "s", $username);
        mysqli_stmt_execute($delStmt);
        $updStmt = mysqli_prepare($db, "UPDATE `student` SET status = 1 WHERE username = ?");
        mysqli_stmt_bind_param($updStmt, "s", $username);
        mysqli_stmt_execute($updStmt);
        // clear session
        unset($_SESSION['email'], $_SESSION['username'],  $_SESSION['student_reset'], $_SESSION['student_reset_time']);

        echo "<script>
          Swal.fire({ title: 'Success!', text: 'Account Activated Successfully.', icon: 'success', confirmButtonText: 'OK' })
        ...
        echo "<script>
          Swal.fire({ title: 'Warning!', text: 'Invalid OTP.', icon: 'warning', confirmButtonText: 'OK' })
               .then(()=>{ window.location='verify_acc'; });
          </script>";
        exit;
    }
}
